...
// void read_parameters(rclcpp::Node::SharedPtr &nh, ConfigSetting &config_setting) {
...
void STDescManager::GenerateSTDescs(
  ...
  // std::cout << "voxel map size: " << voxel_map.size() << std::endl; 
  ...
  // std::cout << "corner points size: " << corner_points->size() << std::endl; 
    ...
    if (!iter->second->plane_ptr_->is_plane_) {
      ...
      for (int i = 0; i < 6; i++) {
        if (current_octo->connect_[i]) {
          ...
          if (current_octo->voxel_points_.size() > 1) {
            ...
            extract_corner(projection_center, projection_normal, proj_points,
                           sub_corner_points);
            std::cout << "sub_corner_points size: " << sub_corner_points->size() << std::endl;
            for (auto pi : sub_corner_points->points) {
              prepare_corner_points->push_back(pi);
            }
            std::cout << "prepare_corner_points size: " << prepare_corner_points->size() << std::endl;
          }
        }
      }
    }
